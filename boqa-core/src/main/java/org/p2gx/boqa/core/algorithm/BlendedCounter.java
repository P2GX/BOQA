...
public class BlendedCounter implements Counter {
    ...
    private Set<TermId> getObservedWithAncestors(Set<TermId> diseaseObservedHpoIds) {
        Set<TermId> phenoIds =  this.hpo.getAllAncestorTermIds(diseaseObservedHpoIds);
        phenoIds.remove(this.hpo.getRootTermId());
        return phenoIds;
    }
     ...
     */
    @Override
    public BoqaCounts computeBoqaCountsFromDisease(Set<TermId> diseaseObservedHpoIds, PatientData patientData) {
        Set<TermId> observedHpos = patientData.getObservedTerms();
        Set<TermId> queryLayer = ontologyTraverser.initLayer(observedHpos);
        Set<TermId> diseaseLayer = getObservedWithAncestors(diseaseObservedHpoIds);
