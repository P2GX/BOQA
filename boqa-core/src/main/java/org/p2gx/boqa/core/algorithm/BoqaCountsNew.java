 ...
 */
public record BoqaCountsNew(int tpBoqaCount, int fpBoqaCount, int tnBoqaCount, int fnBoqaCount) {
    public BoqaCountsNew {
        if (tpBoqaCount < 0 || fpBoqaCount < 0 || tnBoqaCount < 0 || fnBoqaCount < 0) {
            throw new IllegalArgumentException("Counts must be greater than zero!");
        }
    }
}